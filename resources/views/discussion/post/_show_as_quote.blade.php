...
<blockquote class="shadow-sm">
    <div class="quote-header d-flex justify-content-between">
        <div>
            <a href="{{ $post->user->link }}" class="align-middle"><img src="{{ $post->user->avatar_link }}"
                    class="rounded" height="14px"></a>
            <a href="{{ $post->user->link }}"><strong>{{ $post->user->display_name }}</strong></a>
            <small>{{ '@' . $post->user->name }}</small>
        </div>
        <div>
            @if ($post->thread_id !== $current->thread_id)
            <a href="{{ $post->thread->link }}" target="_blank" title="Voir le topic" class="text-small mr-2">
                {{ $post->thread->title }}
            </a>
            @endif
