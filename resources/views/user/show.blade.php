        ...
        <div class="col-md-6">
            <div class="card" style="margin-top: 50px">
                <div class="text-center">
                    <img src="{{ $user->avatar_link }}" class="img-fluid rounded mb-3" width="100"
                        ...
                        <h3 class="h5">Statistiques</h5>
                            ...
                            <h3 class="h5">Sanctions</h5>
                                <div class="p-3 pb-3">
                                    ...
                                    <div class="border rounded mb-1 p-2 bg-theme-tertiary">
                                        ...
                                        <small>le {{ $ban->created_at->format('d/m/Y') }}
                                            ...
                                            / {{ $diffInDays = $ban->created_at->diffInDays($ban->expired_at) }}
                                            {{ Str::plural('jour', $diffInDays) }}
                                            @endif
                                        </small>
                                    </div>
                                    @endforeach
                                ...
                                <h3 class="h5">Dernières participations</h5>
                                    <div class="p-1 pb-3">
                                        @foreach (\App\Models\Post::where('user_id', $user->id)->whereHas('thread',
                                        function($q){$q->public();})->orderBy('updated_at', 'DESC')->limit(10)->get() as
                                        $post)
                                        <a href="{{ $post->link }}">{{ $post->thread->title }}</a><br>
                                        @endforeach
                                    </div>
                                        ...
                                        <div class="p-3 pb-3">
                                            ...
                                            <div
                                                ...
                                                <div data-toggle="tooltip" data-placement="top"
                                                    title="Ce succès est rare"
                                                    class="ribbon pointer ribbon-achievement shadow-lg"></div>
                                                @endif
                                            </div>
                                            @endforeach
                                        </div>
                ...
                <div class="card-footer">
                    <div class="text-right">
                        ...
                        @else
                        <a href="{{ route('private_threads.create', [$user->id, $user->name]) }}"
                            class="btn btn-primary">Envoyer un message privé</a>
                        @if (user()->can('bypass users guard'))
                        <a href="{{ route('user.settings.profile', $user->name) }}"
                            class="btn btn-secondary">Paramètres</a>
                        @endif
                        @if (user()->can('delete users'))
                        <a href="{{ route('user.delete', $user->name) }}" class="btn btn-danger">Supprimer
                            l'utilisateur</a>
                        @endif
                        @endif
                        @endauth
