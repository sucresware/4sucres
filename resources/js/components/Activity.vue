  ...
  <div>
    <div
      :class="{
        ...
        'bg-warning': activity.properties && activity.properties.level == 'warning',
      }"
    >
      <div class="no-gutters text-monospace activity">
        ...
        <b-collapse :id="'context-' + activity.id" class="col-12">
          <div class="card border-bottom border-top">
            <div class="card-body">
              <a :href="subject().link" target="_blank">{{ activity.subject_type }} - {{ activity.subject_id }}</a>
              <hr />
              <div
                v-for="(property, name, $index) in activity.properties"
                :key="name"
                :class="{ 'bg-secondary': $index % 2 }"
              >
                <div class="row flex-nowrap">
                  <div class="text-right col-sm-2">{{ name }}</div>
                  <div class="text-left col-sm">{{ property }}</div>
                </div>
              </div>
            </div>
          </div>
