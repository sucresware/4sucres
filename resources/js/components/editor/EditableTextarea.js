export default {
  methods: {
    ...
    bbWrapWith(pattern, prop) {
      ...
      return this.updateContent(before + selection + after, before.length, before.length + selection.length);
    },
    insert(content) {
      let { text, start, end } = this.getContent();
      let { before, selection, after } = cutTextWithSelection(text, start, end);

      return this.updateContent(before + content + after, before.length, before.length + content.length);
    },
  },
  render() {
