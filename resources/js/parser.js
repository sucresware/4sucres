...
//BBCode
const bbBold = createPlugin(
  /\[b\](.*?)\[\/b\]/,
  (match, utils) => '<strong>' + utils.escape(match[1]) + '</strong>'
)

const bbItalic = createPlugin(
  /\[i\](.*?)\[\/i\]/,
  (match, utils) => '<em>' + utils.escape(match[1]) + '</em>'
)
...
const bbStrike = createPlugin(
  /\[s\](.*?)\[\/s\]/,
  (match, utils) => '<del>' + utils.escape(match[1]) + '</del>'
)
...
const bbImg = createPlugin(
  /\[img\](.*?)\[\/img\]/,
  (match, utils) => '<img src="' + utils.escape(match[1]) + '">'
)
...
const bbMock = createPlugin(
  /\[mock\](.*?)\[\/mock\]/,
  (match, utils) => randomCase(utils.escape(match[1]))
)
...
const bbVapor = createPlugin(
  /\[vapor\](.*?)\[\/vapor\]/,
  (match, utils) => vapor(utils.escape(match[1]))
)
